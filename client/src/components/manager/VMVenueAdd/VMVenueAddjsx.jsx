...
function VMVenueAddjsx() {
  ...
  function handleSubmit(event) {
    ...
    const availableSlots = selectedSlots.map((option) => {
      return {
    ...
    // Send the data to the backend API to save the new turf
  }
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      description: Yup.string()
        .min(4, "Description must be 12 character or higher")
        .max(150, "Descriptioin must be 150 character or lesser")
        .required("Required"),
  ...
  const handleSportChange = (event) => {
    ...
    setSelectedFacility(
      sports.find((perSport) => perSport.sport === sport)?.facilityDetails || []
    );
        ...
        <form onSubmit={formik.handleSubmit}>
          <div class="grid grid-cols-2 gap-4 mb-4 mt-8">
            ...
            <div className="overflow-x-auto whitespace-nowrap">
              <div className="inline-flex space-x-4">
                {availableSlots.map((slot) => (
                  <label
                    ...
                    className="flex items-center space-x-2 bg-gray-100 rounded-lg p-4"
                  >
                    <input
                      type="checkbox"
                      name="slots"
                      ...
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                      checked={formik.values.slots.includes(
                        `${slot.start}-${slot.end}`
                      )}
              ...
              <div className="text-cyan-500">select slots</div>
            </div>
            <div>
          ...
          {mapNotSelectedErrMsg && (
            <p className="text-red-700">Map not selected</p>
          )}
